...
Rules for news posts, blog essays, and other titled prose on HCI Nerdz surfaces.
...
=== Stance (inner vs outer)

The useful divide is not length or polish. It is **where the thought faces**:

[cols="1,2,2"]
|===
| Kind | Faces | Typical cargo

| *News*
| **Outward** — what entered the shared record, what the org (or another actor) did, what readers should notice next
| Ship announcements, repertoire additions, partner notes, status lines

| *Blog / essay*
| **Inward** — ideas, ideals, philosophy, craft tutorials, working theories; often a small social beat (thinking in public)
| Framing essays, “when …” situations, how-I-think pieces, tutorial-shaped reflection
|===

If you are announcing that a thing exists in the world → **news**.
If you are thinking through a claim, ideal, or how to see → **blog**.
Changelog bullets are neither; they stay in changelog / Antora.

House essay on the stance: link:https://ryanjohnson.dev/blog/posts/blog-as-inner-thought/[Blog as inner thought].
...
Do not write an essay title that only announces “we shipped a thing” unless the piece is literally a changelog in essay clothing.

== First-party news: omit the org

On this site, the reader’s attention is already contextualized to HCI Nerdz.
...
Situation (`When …`) titles still have to orient a *cold* reader.
The class of failure must be recognizable before the body scene.
Prefer a claim or job title when the scene only makes sense after you already know the product:
`Making model assumptions transparent`, not a private shorthand for one case study.

== Attach floating modifiers

`Sequence as the top-level` disorients — top-level *what?*
...
When people only *feel* a failure (broken links, rename fallout) but cannot name the system mistake, couple two revisable faces:

[cols="1,2,2"]
|===
| Face | Job | Typical surface

| *Symptom*
| Start from what readers already know and feel
| Situation essay (`When …`), demo that shows the breakage

| *Diagnosis / treatment*
| Name the mistake and the stack of fixes
| Concept docs topic; technical companion essay
|===

Keep the pairing explicit so either face can revise without forcing a retitle of the other.
Example: symptom *When renaming a page breaks half your docs* ↔ diagnosis *Labels versus wires* / *When the name is not the wire*.

Do not steal *Navigating by content* for the reference-identity topic — that title belongs to instruction flows (menu-style docs).

== Quick checklist

. First-party news → org omitted unless another actor?
. Action title → *[On] + title* sounds human? Surplus *the* dropped?
. Abstract claim → *as* / process / *a* / question / *when* / disproof instead of rigid *X is Y*?
. Modifiers attached to an object?
. One idea per news item? Big idea first?
. Sentence case? Orienting out of thin air (including `When …` titles)? Accurate enough?

== Examples for this org
...
| `HCI Nerdz adds instruction flows to the repertoire`
